...
function renderSkills(selectedCat) {
  ...
  skillsToDisplay.forEach(s => {
    ...
    card.innerHTML = `
      <div class="skill-header" style="margin-bottom:0;justify-content:space-between;align-items:center;">
        <div class="skill-icon-name">
          <span class="skill-icon">${s.icon}</span>
          <span class="skill-name">${s.name}</span>
        </div>
        <span class="tag-pill" style="font-size:0.75rem;padding:3px 8px;margin:0;">${s.category}</span>
      </div>
    `;
